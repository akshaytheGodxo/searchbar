...
// CHANGE THIS TO A VALID WINDOWS FONT PATH
constexpr const char* kFontPath = "C:/Windows/Fonts/arial.ttf";
...
int main()
{
    sf::RenderWindow window(
        sf::VideoMode(sf::Vector2u(kWindowWidth, kWindowHeight)),
        "SFML Search Bar"
    );
    ...
    sf::RectangleShape searchBar(
        sf::Vector2f(kSearchBarWidth, kSearchBarHeight)
    );
    ...
    searchBar.setPosition(
        sf::Vector2f(
            (kWindowWidth - kSearchBarWidth) / 2.f,
            (kWindowHeight - kSearchBarHeight) / 2.f
        )
    );
    ...
    inputText.setPosition(
        sf::Vector2f(
            searchBar.getPosition().x + kSearchBarPadding,
            searchBar.getPosition().y + 8.f
        )
    );
    ...
    while (window.isOpen())
    {
        while (const std::optional<sf::Event> event = window.pollEvent())
        {
                         ...
                         event->getIf<sf::Event::MouseButtonPressed>())
            {
                ...
                searchBar.setOutlineColor(
                    isFocused ? sf::Color(120, 180, 255)
                              : sf::Color(90, 90, 90)
                );
            }
        ...
        cursor.setSize(
            sf::Vector2f(kCursorWidth, textBounds.size.y)
        );
        cursor.setPosition(
            sf::Vector2f(
                inputText.getPosition().x + textBounds.size.x + 2.f,
